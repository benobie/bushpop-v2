...
import { DEFAULT_CHANNEL, getChannelConfig, GENDER_LABELS } from "@bushpop/config";
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      ...
      {/*
        ...
        W3 (BF-15): gender is now a first-class IA cut — the Women/Men duo
        below is the default home split (design/HANDOFF-home.md's original
        intent, un-descoped now that `gender` is a real schema field).
        Category duo retired per Ben's W3 call (gender-first, category is
        the second-level filter) — category filtering is still fully
        available via the PLP filter-bar. Counts and images are computed at
...
/**
 * "Shop by gender" duo (W3/BF-15) — Women / Men, each tile showing a real
 * cover image + a live count. Renders nothing unless BOTH buckets have at
 * least one listing (mirrors the retired category duo's <2 rule — an
 * honest empty state per the trust-claims ledger, no half-filled duo, no
 * fabricated gender). Unisex/kids don't get home tiles — they're still
 * fully reachable via the PLP gender filter.
 */
async function GenderDuo() {
  await connection();
  const [women, men] = await Promise.all([
    browseListings({ channel: DEFAULT_CHANNEL, gender: "women", limit: 1 }),
    browseListings({ channel: DEFAULT_CHANNEL, gender: "men", limit: 1 }),
  ]);

  const tiles = [
    { gender: "women" as const, name: GENDER_LABELS.women, count: women.total, imageUrl: women.items[0]?.primaryImageUrl ?? null },
    { gender: "men" as const, name: GENDER_LABELS.men, count: men.total, imageUrl: men.items[0]?.primaryImageUrl ?? null },
  ];
  if (tiles.some((tile) => tile.count === 0)) return null;

  return (
    <div className="mb-12">
      <h2 className="mb-4 font-display text-xl font-semibold text-bp-ink">Shop by gender</h2>
      <div className="grid grid-cols-2 gap-4">
        {tiles.map((tile) => (
          <Link
            key={tile.gender}
            href={`/shop?gender=${tile.gender}`}
