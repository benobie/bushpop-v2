...
import { getChannelConfig } from "@bushpop/config";
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      ...
      <Suspense fallback={<LatestListingsSkeleton />}>
        <LatestListings channel={channel} />
      </Suspense>
    </main>
  );
}

async function LatestListings({ channel }: { channel: string }) {
  await connection();
  const result = await browseListings({ channel, limit: 8 });
