...
export default async function DashboardListingsPage({ params, searchParams }: DashboardListingsPageProps) {
    ...
    <main className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {STATUS_FILTERS.map((s) => (
          <Link
            key={s}
            href={s === "all" ? `/${channel}/dashboard/listings` : `/${channel}/dashboard/listings?status=${s}`}
            className={[
      ...
      {listings.length > 0 && (
        <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
          {listings.map((listing) => (
            <Link
              key={listing.id}
              href={`/${channel}/sell/${listing.inventoryItemId}/review`}
