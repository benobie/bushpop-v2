...
#
# Asserts every `'use cache'` function in apps/web/src/ has at least one
...
CACHED_FILES=$(grep -rlE "^[[:space:]]*['\"]use cache['\"]" apps/web/src/ 2>/dev/null || true)
CACHED_COUNT=0
...
INVALIDATED_COUNT=$( { grep -rnE "revalidateTag\(" apps/web/src/ packages/ 2>/dev/null || true; } \
  | grep -v "\.test\." \
  | grep -v "/node_modules/" \
  | grep -vE ":[[:space:]]*\*[[:space:]]" \
  | grep -vE ":[[:space:]]*//" \
...
echo "  'use cache' files in apps/web/src/: ${CACHED_COUNT}"
