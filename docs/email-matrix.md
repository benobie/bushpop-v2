...
| 3 | Buyer shipping confirmation (with tracking) | Buyer | `shippingConfirmationBuyerTemplate` | `order.shipped` domain event (four producers since 08/07 — see §2 below) → `event-consumer.ts` → `enqueueEmail` | YES | `test/integration/email/email.test.ts` (template + send) + `test/integration/shipping/shipping.test.ts` (event dispatch incl. webhook producer + redelivery idempotency) |
...
**Correction 08/07/2026 — there were FOUR `paid → shipped` writers, not two.** The §2 claim above missed that the Starshipit **webhook** `dispatched` case (`routes/v1/webhooks/starshipit.ts`) and the Starshipit **poll worker** `dispatched` case (`workers/starshipit-poll.ts`) each perform their own CAS `paid → shipped` without dispatching `order.shipped` — so an order whose transition point was the webhook (label created out-of-band, or the label worker crashed between label creation and its DB write) reached `shipped` with **no buyer email**. The poll leg was unreachable in practice (the poll query only selects already-`shipped` orders) but would silently reactivate the gap if that query ever broadened. Both now dispatch `order.shipped` on their CAS win, gated exactly like the other two producers, so redelivery/races stay at-most-once; the webhook also persists the payload's `tracking_number` when the order has none (the email worker requires one). Its payload carries **no carrier field**, so `shippingConfirmationBuyerTemplate`'s `trackingCarrier` is now optional — the Carrier line is omitted rather than inventing one or dead-lettering the email. Rides along: buyer-facing sends (rows 1, 3, 4) are skipped with a warn log when the buyer email is an anonymous-guest placeholder (`@guest.bushpop.com.au`, BF-08 guest bag) — those addresses are undeliverable and would bounce against Resend's sender reputation; PR #106's `setGuestCheckoutEmail` will capture real guest emails at checkout, and the guard is the marked integration site for the guest order-access link.
