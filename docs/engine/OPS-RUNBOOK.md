...
- **R2 bucket CORS** (fixed 08/07, batch 45 session C) is a bucket-level setting, not a compose env var — set via `wrangler r2 bucket cors set bushpop-images --file <json>` (cached OAuth, account `5be04ea84f417fdc80bfcd40b9919fc2`), currently allowing `GET,PUT,HEAD` from `market.bushpop.xyz`/`bushpop.com.au`/`www.bushpop.com.au`. It had NO CORS config at all before this fix, which silently broke every direct-to-R2 browser upload (`/sell` photos, `/bulk-listing`) with a generic "Failed to fetch". Survives Coolify redeploys (not compose-managed) — but remember to add the exact production origin here at cutover if it isn't already covered by the three above.
