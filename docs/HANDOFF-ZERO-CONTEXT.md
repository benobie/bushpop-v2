...
| 04/07/2026 | **PROPOSED, pending Ben:** no runtime `PAYMENTS_MODE` flag — `/checkout` already IS Phase-1 direct mode by construction (absence of Connect fields, not a config value); `/checkout-groups` should be env-gated OFF by default (mirror `PAYOUT_RELEASE_ENABLED`, `workers/index.ts:92-104`) until Phase 2 builds its missing webhook/expiry/reconciliation legs | bushpop-q10 session C (Claude Code, Sonnet 5) | this PR's body ("docs: direct-mode formalisation memo"); `checkout/service.ts:287-302`; `checkout-groups/service.ts:252,445-451`; `server.ts:194`; `routes/v1/webhooks/stripe.ts` (no `orderGroup` matches); `workers/reconcile-indeterminate-ops.ts:77` |
